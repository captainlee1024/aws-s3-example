...
async fn main() -> Result<(), Error> {
    // first set the environment variables of credentials
    env::set_var("AWS_ACCESS_KEY_ID", "test");
    env::set_var("AWS_SECRET_ACCESS_KEY", "test");

    // second, set the environment variables of the region
...
async fn initialize_variables() -> (Region, Client, String, String, String, String) {
    ...
    let shared_config = aws_config::from_env().region(region_provider).endpoint_url(LOCALSTACK_ENDPOINT).load().await;
...
fn s3_client(conf: &aws_config::SdkConfig) -> aws_sdk_s3::Client {
    ...
    // let mut s3_config_builder = aws_sdk_s3::config::Builder::from(conf).set_force_path_style(Some(true));
    let mut binding = aws_sdk_s3::config::Builder::from(conf);
    let mut s3_config_builder = binding.set_force_path_style(Some(true));
    aws_sdk_s3::Client::from_conf(s3_config_builder.clone().build())
}
